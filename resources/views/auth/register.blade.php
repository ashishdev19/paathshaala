    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4" id="registerForm">
        ...
        <div>
            <label for="referral_code" class="block text-sm font-medium text-gray-700 mb-1.5">
                <i class="fas fa-gift text-gray-400 mr-2"></i>Referral Code <span class="text-gray-400 font-normal">(Optional)</span>
            </label>
            <input id="referral_code" type="text" name="referral_code" value="{{ old('referral_code', request('ref')) }}"
                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('referral_code') border-red-500 @enderror"
                   placeholder="Enter referral code to get ₹100 discount">
            @error('referral_code')
                <p class="mt-1 text-sm text-red-600"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
            @else
                <p class="mt-1 text-sm text-teal-600"><i class="fas fa-info-circle mr-1"></i>Get instant ₹100 discount on your first purchase!</p>
            @enderror
        </div>
