    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg border border-indigo-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-indigo-100 bg-gradient-to-r from-indigo-50 to-purple-50">
                <h3 class="text-lg font-semibold text-indigo-900">Update Category Information</h3>
                <p class="text-sm text-gray-600">Edit the details below to update the course category</p>
            </div>
            ...
            <form action="{{ route('admin.course-categories.update', $courseCategory) }}" method="POST" class="p-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div id="iconPreview" class="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center">
                            <i class="fas {{ $courseCategory->icon_class }} text-indigo-600 text-xl"></i>
                        </div>
                    </div>
                    @error('icon')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-6">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input 
                            ...
                            value="1"
                            {{ old('show_on_homepage', $courseCategory->show_on_homepage) ? 'checked' : '' }}
                ...
                <div class="mb-6">
                    ...
                    <input 
                        ...
                        name="display_order" 
                        id="display_order" 
                        value="{{ old('display_order', $courseCategory->display_order) }}"
                        min="0"
                ...
                <div class="mb-6">
                    ...
                    <select 
                        ...
                        required
                    >
                        <option value="active" {{ old('status', $courseCategory->status) === 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $courseCategory->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <span class="font-semibold text-gray-900 ml-2">{{ $courseCategory->courses()->count() }}</span>
                        </div>
                        <div>
                            <span class="text-gray-600">Created:</span>
                            <span class="font-semibold text-gray-900 ml-2">{{ $courseCategory->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
