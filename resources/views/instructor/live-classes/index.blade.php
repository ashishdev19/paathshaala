<x-layouts.instructor>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-indigo-50 to-purple-50">
            ...
            <a href="{{ route('instructor.live-classes.create') }}" class="bg-gradient-to-r from-green-600 to-green-700 text-white px-4 py-2 rounded-lg hover:from-green-700 hover:to-green-800 transition font-semibold shadow-md">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($liveClasses as $index => $class)
                    @php
                    ...
                    @endphp
                    <tr class="hover:bg-gray-50 transition duration-200 {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }}">
                        ...
                        <td class="px-5 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ $class->topic }}</div>
                            @if($class->description)
                                <div class="text-xs text-gray-500 mt-1">{{ Str::limit($class->description, 50) }}</div>
                            @endif
                        </td>
                        <td class="px-5 py-4 whitespace-nowrap">
                            <div class="flex items-center text-sm text-gray-900 font-medium">
                                ...
                                {{ $class->start_datetime->format('M d, Y') }}
                            </div>
                            <div class="flex items-center text-xs text-gray-500 mt-1">
                                ...
                                {{ $class->start_datetime->format('h:i A') }}
                            </div>
                        </td>
                        <td class="px-5 py-4 whitespace-nowrap">
                        ...
                        <td class="px-5 py-4 whitespace-nowrap">
                            <span class="px-3 py-1 text-xs font-semibold rounded-full 
                                @if($class->mode === 'online') bg-blue-100 text-blue-800 border border-blue-200
                                @elseif($class->mode === 'offline') bg-purple-100 text-purple-800 border border-purple-200
                        ...
                        <td class="px-5 py-4 whitespace-nowrap">
                            <div class="relative group">
                                <button class="px-3 py-1 text-xs font-semibold rounded-full inline-flex items-center
                                    @if($status === 'upcoming') bg-indigo-100 text-indigo-800 border border-indigo-200
                                    @elseif($status === 'live') bg-green-100 text-green-800 border border-green-200 animate-pulse
                                    @elseif($status === 'completed') bg-gray-100 text-gray-800 border border-gray-300
                                    @else bg-red-100 text-red-800 border border-red-200 @endif">
                                    <svg class="w-3 h-3 mr-1.5" fill="currentColor" viewBox="0 0 8 8">
                                        <circle cx="4" cy="4" r="2.5"/>
                                    </svg>
                                    @if($status === 'upcoming') Upcoming
                                    @elseif($status === 'live') Live Now
                                ...
                                <div class="absolute right-0 mt-1 w-40 bg-white rounded-lg shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-20">
                                    <div class="p-3 space-y-2">
                                        <button onclick="updateStatus({{ $class->id }}, 'upcoming')" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-indigo-50 rounded-lg transition duration-150 flex items-center">
                                            <span class="inline-block w-2 h-2 rounded-full bg-indigo-600 mr-2"></span>
                                            Upcoming
                                        </button>
                                        <button onclick="updateStatus({{ $class->id }}, 'live')" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded-lg transition duration-150 flex items-center">
                                            <span class="inline-block w-2 h-2 rounded-full bg-green-600 mr-2 animate-pulse"></span>
                                            Live Now
                                        </button>
                                        <button onclick="updateStatus({{ $class->id }}, 'completed')" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition duration-150 flex items-center">
                                            <span class="inline-block w-2 h-2 rounded-full bg-gray-600 mr-2"></span>
                                            Completed
                                        </button>
                                        <button onclick="updateStatus({{ $class->id }}, 'cancelled')" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded-lg transition duration-150 flex items-center">
                        ...
                        <td class="px-5 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-2">
                                <!-- View Button -->
                                <a href="{{ route('instructor.live-classes.show', $class->id) }}" 
                                ...
                                <!-- Edit Button -->
                                <a href="{{ route('instructor.live-classes.edit', $class->id) }}" 
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center">
                            ...
                            <a href="{{ route('instructor.live-classes.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 shadow-md font-semibold transition duration-300">
